...
class Sequential:
    ...
    def train(self, X_train, y_train, loss_fn, optimizer, epochs=100, batch_size=32,
        ...
        for epoch in range(epochs):
            ...
            for X_batch, y_batch in get_batches(X_train, y_train, batch_size, shuffle=True):
                ...
                # Compute accuracy
                correct_preds += accuracy(y_batch, torch.argmax(y_pred, dim=1)) * X_batch.shape[0]
    ...
    def evaluate(self, X, y, loss_fn):
        ...
        # Compute accuracy
        accuracy_value = accuracy(y, torch.argmax(y_pred, dim=1))
    ...
    def to(self, device):
        """
        Move all layer parameters to the specified device.
        
        Parameters:
        - device: torch.device
        """
        for layer in self.layers:
            # Handle Layer parameters (assuming parameters stored as a dict)
            if hasattr(layer, 'parameters'):
                for key, param in layer.parameters.items():
                    if isinstance(param, torch.Tensor):
                        layer.parameters[key] = param.to(device)
            # You can add additional logic if Activations/other attributes need moving
